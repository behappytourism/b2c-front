...
import { TicketIcon, SparklesIcon, RocketLaunchIcon } from "@heroicons/react/24/outline";
import { TicketIcon as SolidTicketIcon, SparklesIcon as SolidSparkle, RocketLaunchIcon as SolidRocket } from "@heroicons/react/24/solid";
import TransferSearchForm from "@/app/transfer/TransferSearchForm";


export type SearchTab = "Stays" | "Experiences" | "Visa" | "Flights" | "Transfer";
...
const HeroSearchForm: FC<HeroSearchFormProps> = ({
  ...
  const tabs: SearchTab[] = ["Experiences", "Transfer"]
      ...
      <ul className="ml- sm:ml- md:ml- hidden md:flex space-x-5 sm:space-x-8 lg:space-x-3 overflow-x-auto hiddenScrollbar">
        {tabs.map((tab) => {
          ...
          const renderIcons = (tab: SearchTab) => {
            switch (tab) {
              case 'Experiences':
                return tabActive === "Experiences" ? <SolidSparkle /> : <SparklesIcon color="black"  className="dark:text-white" />
              case 'Flights':
              ...
              case 'Transfer':
                  return tabActive === "Transfer" ? <SolidRocket /> : <RocketLaunchIcon color="black" className="dark:text-white" />
              default:
                return null;
            }
          };
          ...
          return (
            <li
              ...
              className={`flex-shrink-0 flex h-fit items-center px-4 py-3 cursor-pointer text-sm lg:text-base font-medium ${active
                ...
                : "text-black bg-gray-200 dark:bg-neutral-800 dark:text-white hover:bg-gray-300 dark:hover:text-neutral-200 rounded-xl"
                } `}
              key={tab}
            >
