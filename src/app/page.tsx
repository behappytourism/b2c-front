...
const PageHome = () => {
  ...
  const { attractionDestinations, globalData } = useSelector((state: RootState) => state.initials)

  const findAttraction = async () => {
    try {
      // const attraction = await fetch('https://jsonplaceholder.typicode.com/todos/1')
      const attraction = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/v1/attractions/all?limit=8&skip=0&destination=${dest}`, { next: { revalidate: 1 } })
    ...
    <main className="nc-PageHome relative overflow-hidden">
      ...
      <div className="relative md:mb-16 mb-16 lg:mb-16">
        ...
        <div className=" relative container space-y-10 lg:space-y-12 mt-10 lg:mt-28">
          ...
          {/* <SectionOurFeatures /> */}
          {attractionData && (
            <SectionGridFilterAttractionCard data={attractionData} tabs={tabs} setDest={setDest} className="md:pb-24 lg:pb-28" />
          )}
