...
function TransferList() {
  ...
  const { transfer, transferCart } = useSelector((state: RootState) => state.transfer);
  ...
  const handleAddToCart = (vehicle: any, trip: any, transfer: TransferExcursion) => {
  ...
  const handleCheckout = (vehicle: any, trip: any, transfer: TransferExcursion) => {
    const selectedTransferItem = {
      _id: vehicle?._id,
      date: trip?.date,
      time: trip?.time,
            ...
            <div className="flex justify-between p-1">
              <div className="">
                ...
                <h1 className="text-xl font-semibold">
                  {trip?.transferFrom?.airportName || trip?.transferFrom?.name}
                </h1>
              </div>
              ...
              <div className="">
                ...
                <h1 className="text-xl font-semibold">
                  {trip?.transferTo?.name || trip?.transferFrom?.airportName}
                </h1>
              </div>
            </div>
              ...
              <div className="border w-fit cursor-pointer  bg-slate-100 transform hover:scale-110 transition-transform duration-300">
                ...
                <div className="p-5">
                  ...
                  <div className="flex gap-2">
                    ...
                    <p className="font-semibold">{priceConversion(vehicle?.price, selectedCurrency, true)}</p>
                  </div>
                  ...
                  <div className="flex flex-col mt-3 gap-3 font-semibold">
                    <button onClick={() => handleAddToCart(vehicle, trip, transferItem)} className="border p-2 border-orange-500 text-orange-500 hover:border-orange-700 hover:text-orange-700">
                      Add To Cart
                    </button>
                    <button onClick={() => handleCheckout(vehicle, trip, transferItem)} className="border p-2 hover:bg-orange-700 bg-orange-500  text-white">
                      Checkout
                    </button>
                  </div>
                </div>
